...
static class InvokeDirectExtension
{
    ...
    private static readonly Dictionary<string, IInvokeDirect> _invokers = [];
    ...
    public static object InvokeDirect(this MethodInfo method, object instance, params object[] arguments)
    {
        ...
        lock (_invokers)
        {
            ...
            if (!_invokers.TryGetValue(sig, out invoker))
            {
                ...
                invoker = _invokers[sig] = (IInvokeDirect) Activator.CreateInstance(classType);
            }
        }
    ...
    {
        var typeBuilder = _modBuilder.DefineType("RT.Servers.InvokeDirect.GeneratedClass." + counter, TypeAttributes.Public,
            typeof(object), [typeof(IInvokeDirect)]);
        ...
        var methodBuilder = typeBuilder.DefineMethod("DoInvoke", MethodAttributes.Public | MethodAttributes.Virtual,
            typeof(object), [typeof(object), typeof(object[]), typeof(MethodInfo)]);
        ...
        il.Emit(OpCodes.Call, typeof(Delegate).GetMethod("CreateDelegate", [typeof(Type), typeof(object), typeof(MethodInfo)]));
    ...
    {
        var delegateBuilder = _modBuilder.DefineType("RT.Servers.InvokeDirect.GeneratedDelegate." + counter,
    ...
    private static void createAssembly()
    {
        if (_asmBuilder == null)
        {
            _asmBuilder = AssemblyBuilder.DefineDynamicAssembly(new AssemblyName("RT.Servers.InvokeDirect.DynamicAssembly"), AssemblyBuilderAccess.Run);
            _modBuilder = _asmBuilder.DefineDynamicModule("RT.Servers.InvokeDirect.DynamicAssembly.Module");
        }
    }
