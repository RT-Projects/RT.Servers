...
namespace RT.Servers
{
    ...
    public sealed class HttpServerOptions
    {
        ...
        /// <remarks>
        ///     This is a string rather than System.Net.IPAddress so that it is reasonably XmlClassifyable. If the contents don’t
        ///     parse, null is assumed.</remarks>
        public string BindAddress = null;

        /// <summary>
        ///     The port on which the server should listen for unsecured HTTP connections. Default is 80. Set to <c>null</c> to
        ///     disable unsecured HTTP.</summary>
        public int? Port = 80;

        /// <summary>
        ///     The port on which the server should listen for secured HTTPS connections. Default is <c>null</c>, i.e. with HTTPS
        ///     disabled. The usual port for HTTPS is 443.</summary>
        ...
        /// <summary>
        ///     The maximum size (in bytes) at which file uploads in a POST request are stored in memory. Any uploads that exceed
        ///     this limit are written to temporary files on disk. Default is 16 MB.</summary>
        ...
        ///     gzip is requested in the HTTP request). Default is 1 MB. Content larger than this size will be gzipped in chunks
        ...
        ///     determine whether gzipping the whole file is worth it. Otherwise it will default to using gzip either
        ///     way.</summary>
        public int GzipAutodetectThreshold = 1024 * 1024;

        /// <summary>The temporary directory to use for file uploads in POST requests. Default is <see cref="Path.GetTempPath"/>.</summary>
        public string TempDir = Path.GetTempPath();

        /// <summary>
        ///     Determines whether the default error handler outputs exception information (including a stack trace) to the client
        ///     (browser) as HTML. The default error handler always outputs the HTTP status code description. It is invoked when
        ///     <see cref="HttpServer.ErrorHandler"/> is null or throws an exception.</summary>
