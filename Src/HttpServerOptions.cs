...
public sealed class HttpServerOptions : IClassifyObjectProcessor
{
    ...
    private readonly Dictionary<string, HttpEndpoint> _endpoints = new(StringComparer.OrdinalIgnoreCase) { ["HTTP"] = new HttpEndpoint("127.0.0.1", 8990) };
    ...
    ///             cref="_certificatePath"/> is <c>null</c>.</description></item></list></remarks>
    ...
    {
        if (_bindAddress != null || _port != null || _securePort != null)
        {
            if (_port != null)
                AddEndpoint("HTTP", _bindAddress, _port.Value, secure: false);
            if (_securePort != null)
                AddEndpoint("HTTPS", _bindAddress, _securePort.Value, secure: true);
        ...
        if (Certificate == null && _certificatePath != null)
        {
            Certificate = new HttpServerCertificateInfo { Path = _certificatePath, Password = _certificatePassword };
...
public sealed class HttpServerCertificateInfo
{
    ...
    public X509Certificate2 GetCertificate() => new(Path, Password);
}
