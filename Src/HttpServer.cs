...
public class HttpServer
{
    ...
    ///     <see cref="HttpServerOptions.Endpoints"/>, <see cref="HttpServerOptions._certificatePath"/>.</remarks>
    ...
    private sealed class ConnectionHandler
    {
        ...
        private void beginAuthenticateAsServer(HttpServer server)
        {
            ...
            var certificate = sniHost.NullOr(host => server.Options.CertificateResolver?.Invoke(host) ?? server.Options.Certificates?.Get(host, null)?.GetCertificate())
            ...
            finally
            {
                try
                {
                    if (originalRequest.FileUploads != null)
                        foreach (var fileUpload in originalRequest.FileUploads.Values.Where(fu => fu._localFilename != null && !fu._localFileMoved))
                            File.Delete(fileUpload._localFilename);
                }
                catch (Exception) { }
            }
        }
        ...
        {
            string[] lines = _headersSoFar.Split(["\r\n"], StringSplitOptions.None);
