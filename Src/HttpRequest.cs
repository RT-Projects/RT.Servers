...
{
    private NameValuesCollection<string> _postFields = [];
    private Dictionary<string, FileUpload> _fileUploads = [];
            ...
            if (bytesRead > 0)
            {
                ...
                else if (bytesRead >= lastBoundary.Length)   // processing content
                {
                    ...
                    if (howMuchToWrite > 0 && currentWritingStream != null)
                    {
                        ...
                        if (currentIsFileUpload && currentWritingStream is MemoryStream memory && memory.Length + inMemoryFileUploadsTotal + howMuchToWrite > storeFileUploadInFileAtSize)
                        {
                            ...
                            if (inMemoryKeys.Count > 0 && memory.Length < inMemoryKeys[inMemoryKeys.Count - 1])
                            {
                                ...
                                biggestUpload._localFilename = HttpInternalObjects.RandomTempFilepath(tempPath, out var fileStream);
                                fileStream.Write(biggestUpload._data, 0, biggestUpload._data.Length);
                                fileStream.Close();
                                fileStream.Dispose();
                                inMemoryFileUploadsTotal -= biggestUpload._data.LongLength;
                                biggestUpload._data = null;
                        ...
                        if (currentWritingStream != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (currentFileUploadTempFilename != null)
                                    ...
                                    fileUpload._localFilename = currentFileUploadTempFilename;
                                else
                                {
                                    ...
                                    fileUpload._data = memory.ToArray();
                                    inMemoryFileUploads.AddSafe(fileUpload._data.LongLength, fileUpload);
                                    inMemoryFileUploadsTotal += fileUpload._data.LongLength;
                                }
                    ...
                    if (currentWritingStream != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (currentFileUploadTempFilename != null)
                                ...
                                fileUpload._data = ((MemoryStream) currentWritingStream).ToArray();
                            _fileUploads[currentFieldName] = fileUpload;
                        }
