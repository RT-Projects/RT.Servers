...
///         <item><description>
///             Derive that class from <see cref="Session"/>, <see cref="FileSession"/>, or your own specialization of either.</description></item>
///         <item><description>
///             <para>
...
public abstract class Session
{
    ...
    ///     cref="Session.EnableAutomatic{TSession}(HttpRequest, Func{TSession, HttpResponse})"/>, <see
    ...
    ///     <c>true</c> before the request handler returns if any change was made to the session. If your session class does
    ///     not have a default constructor, use <see cref="SessionExtensions.EnableManual{TSession}(TSession, HttpRequest,
    ///     Func{TSession, HttpResponse})"/>.</summary>
    /// <typeparam name="TSession">
    ///     The type of session to be used.</typeparam>
    /// <param name="req">
    ///     The HTTP request for which to enable session support.</param>
...
public enum SessionAction
{
    ...
    /// <summary>
    ///     Cause any changes to the session to be ignored. The next request will receive the previous state of the session.</summary>
    DoNothing,

    /// <summary>
    ///     Cause the session and the associated cookie to be deleted. The deletion occurs in <see cref="Session.CleanUp"/>.</summary>
    Delete
}
...
public static class SessionExtensions
{
    ...
    /// <summary>
    ///     Enables the use of sessions in an HTTP request handler. Use this if your session class does not implement <see
    ///     cref="ISessionEquatable{TSession}"/>; your class will have to manually set <see cref="Session.SessionModified"/>
    ///     to <c>true</c> before the request handler returns if any change was made to the session.</summary>
    /// <typeparam name="TSession">
    ///     The type of session to be used.</typeparam>
    /// <param name="session">
    ///     An already-constructed empty session object. The session object will have <see cref="Session.newSession"/> or <see
