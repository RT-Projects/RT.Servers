...
/// <summary>
///     Encapsulates a response to an HTTP request. Concrete classes implementing this are <see cref="HttpResponseContent"/>
///     and <see cref="HttpResponseWebSocket"/>.</summary>
public abstract class HttpResponse(HttpResponseHeaders headers) : MarshalByRefObject
{
    ...
    public HttpResponseHeaders Headers { get { return headers; } }
    ...
    ///     If specified, a 304 Not Modified will be served if the file's last modified timestamp is at or before this time.</param>
    public static HttpResponseContent File(string filePath, string contentType = null, int? maxAge = 3600, DateTime? ifModifiedSince = null)
    {
        try
        {
            ...
            return create(getFileStream, contentType, HttpStatusCode._200_OK, new HttpResponseHeaders
            {
                ...
                CacheControl = maxAge == null ? null : [new HttpCacheControl { State = HttpCacheControlState.MaxAge, IntParameter = maxAge.Value }],
            });
        }
    ...
    public static HttpResponseContent Redirect(string newUrl)
    {
        return new HttpResponseContent(
            ...
            {
                Location = newUrl,
                CacheControl = [new HttpCacheControl { State = HttpCacheControlState.Private }, new HttpCacheControl { State = HttpCacheControlState.MaxAge, IntParameter = 0 }],
            });
    }
    ...
    ///     Headers to use in the response, or null to use default values.</param>
    /// <param name="buffered">
    ///     If true (default), the output is buffered for performance; otherwise, all text is transmitted as soon as possible.</param>
    ...
    /// <summary>
    ///     Returns the specified content to the client as a single concatenated piece of text with the MIME type “text/html;
    ///     charset=utf-8”.</summary>
    /// <param name="content">
    ///     Content to return to the client.</param>
    /// <param name="status">
    ///     HTTP status code to use in the response.</param>
    ...
    /// <summary>
    ///     Returns the specified content to the client as a single concatenated piece of text with the MIME type “text/plain;
    ///     charset=utf-8”.</summary>
    /// <param name="content">
    ///     Content to return to the client.</param>
    /// <param name="status">
    ...
    /// <summary>
    ///     Returns the specified content to the client as a single concatenated piece of text with the MIME type “text/css;
    ///     charset=utf-8”.</summary>
    /// <param name="content">
    ///     Content to return to the client.</param>
    /// <param name="status">
    ...
    /// <summary>
    ///     Returns the specified content to the client as a single concatenated piece of text with the specified MIME type.</summary>
    /// <param name="content">
    ///     Content to return to the client.</param>
    /// <param name="contentType">
    ///     MIME type to use. This overrides any MIME type specified in <paramref name="headers"/> (if any).</param>
    ...
    /// <summary>
    ///     Provides an implicit conversion from <see cref="RT.Servers.WebSocket"/> to <see cref="HttpResponse"/> by assuming
    ///     no subprotocol and no extra HTTP response headers.</summary>
    /// <param name="websocket">
