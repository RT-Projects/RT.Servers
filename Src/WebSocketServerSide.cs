...
internal sealed class WebSocketServerSide : MarshalByRefObject
{
    ...
    private static readonly byte[] _pong = [0x8a, 0];
    private static readonly byte[] _finalFrameZeroPayload = [0x80, 0];
    ...
    public void SendMessageFragmentEnd(byte opcode)
    {
        withExceptionHandling(() =>
        {
            _socketStream.Write(opcode == 0 ? _finalFrameZeroPayload : [(byte) (0x80 | opcode), 0]);
        });
    }
